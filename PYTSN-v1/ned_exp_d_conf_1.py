...
# 3 <-> 7
tcps_3 = tcps(pytsn,id=3,lan_id=1,dest_id=7,initial_delay=0e-3,priority=0,rate=1000,\
	pkt_size=100,burst_count=1250,finish=10,debug=True) #burst_count=10=>10Mbps

tcps_7 = tcps(pytsn,id=7,lan_id=1,dest_id=3,initial_delay=0e-3,priority=0,rate=1000,\
	pkt_size=100,burst_count=1250,finish=10,debug=True)

# 8 <-> 5
tcps_8 = tcps(pytsn,id=8,lan_id=1,dest_id=5,initial_delay=0e-3,priority=0,rate=1000,\
	pkt_size=100,burst_count=1250,finish=10,debug=True)

tcps_5 = tcps(pytsn,id=5,lan_id=1,dest_id=8,initial_delay=0e-3,priority=0,rate=1000,\
	pkt_size=100,burst_count=1250,finish=10,debug=True)
...
be_10 = terminal(pytsn,id=10,lan_id=1,dest_id=10,flow_id= -1,priority=0,\
	adist=dist_be.adist,sdist=dist_be.sdist,initial_delay=0,rate=1000,debug=True)
...
sw_0 = switch_ds(pytsn,sw_0_fwd_tbl,txq_limit=1000,pro_delay=0.5e-6,rate=1000,\
	...
	scl_gl_tcps=scl_gl_tcps,scl_gbnd=scl_gbnd,scl_ns=scl_ns)

sw_1_fwd_tbl = {1:1, 2:1, 3:1, 4:4, 5:4, 6:4, 7:2, 8:3, 9:4, 10:4} # dst-id:output-port
sw_1 = switch_ds(pytsn,sw_1_fwd_tbl,txq_limit=1000,pro_delay=0.5e-6,rate=1000,\
	list_prt_types=["BE","ST","ST","BE"],scl_gp=scl_gp,scl_gl_netm=scl_gl_netm,\
	scl_gl_tcps=scl_gl_tcps,scl_gbnd=scl_gbnd,scl_ns=scl_ns)

sw_2_fwd_tbl = {1:1, 2:1, 3:1, 4:4, 5:4, 6:4, 7:1, 8:1, 9:2, 10:3} # dst-id:output-port
...
sw_3_fwd_tbl = {1:1, 2:1, 3:1, 4:4, 5:2, 6:3, 7:1, 8:1, 9:1, 10:1} # dst-id:output-port
sw_3 = switch_ds(pytsn,sw_3_fwd_tbl,txq_limit=1000,pro_delay=0.5e-6,rate=1000,\
	list_prt_types=["BE","ST","BE","ST"],scl_gp=scl_gp,scl_gl_netm=scl_gl_netm,\
	scl_gl_tcps=scl_gl_tcps,scl_gbnd=scl_gbnd,scl_ns=scl_ns)
...
link_list[6].output = sw_1.p2_in
sw_1.p2_out = link_list[7].input
link_list[7].output = tcps_7.input

# id:8 to sw-1(p3)
tcps_8.output = link_list[8].input
link_list[8].output = sw_1.p3_in
sw_1.p3_out = link_list[9].input
...
link_list[12].output = sw_2.p3_in
sw_2.p3_out = link_list[13].input
...
link_list[18].output = sw_3.p4_in
sw_3.p4_out = link_list[19].input
